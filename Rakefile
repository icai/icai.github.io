...
task :watch do
  ...
  jekyllPid = Process.spawn({"JEKYLL_ENV"=>"development"}, "jekyll build  --watch")
...
task :preview do
  ...
  # system "compass compile --css-dir #{source_dir}/stylesheets" unless File.exist?("#{source_dir}/stylesheets/screen.css")
  jekyllPid = Process.spawn({"OCTOPRESS_ENV"=>"preview"}, "jekyll serve --host 0.0.0.0 --port #{server_port} --livereload")
  # compassPid = Process.spawn("compass watch")
...
task :copydot, :source, :dest do |t, args|
  FileList["#{args.source}/**/.*"].exclude("**/.", "**/..", "**/.jekyll-metadata", "**/.DS_Store", "**/._*", "**/.bower.json").each do |file|
